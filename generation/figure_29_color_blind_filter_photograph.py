"""
Filtering a colorful photograph to mimic the effects of color-blindness, with
filtered images and chromatic distributions.

Caption: A colorful photograph filtered to remove chromatic differences to which
an observer with a missing cone type would not be sensitive.  From left-to-right
are the original image, a protanope filtered image, a deuteranope filtered
image, and a tritanope filtered image.  Below each image is a chromaticity
diagram showing the distribution of chromaticities in the image.  Note that the
distributions of filtered images have an arc shape - the center around which the
distribution arcs is the corresponding copunctal point for the missing cone
type.
"""
...
original_image = Image.open('images/figure_29_photograph.jpg')
...
figure = Figure(
    name = 'figure_29_color_blind_filter_photograph{0}'.format(
