...
\backgroundsetup{
    scale=1,
    angle=0,
    contents={\includegraphics[width=\paperwidth,height=\paperheight]{../images/figure_00_title_background_inverted.pdf}}
}
...
\backgroundsetup{
    scale=1,
    angle=0,
    contents={\includegraphics[width=\paperwidth,height=\paperheight]{../images/figure_00_title_background.pdf}}
}
\fi
...
% Paper width is \printinunitsof{in}\prntlen{\paperwidth}\\
% Paper height is \printinunitsof{in}\prntlen{\paperheight}
...
\begin{figure*} % Figure 4
    ...
    \fi
    \caption{Normalized, smoothed cone fundamentals.  The approximate peak of each cone's sensitivity curve is annotated.  (generation \href{https://github.com/kyle-c-mcdermott/visualizing-color-space/blob/main/generation/figure_04_cone_fundamentals.py}{code} and images \href{https://drive.google.com/drive/folders/1C8dVtqOiEoHkGyonFmBHFd7FEmq-E7p0?usp=sharing}{folder})}\label{fig:cone_fundamentals}
\end{figure*}
...
\begin{halffigure} % Figure 5
    ...
    \fi
    \captionof{figure}{CIE 170-2 10$^\circ$ color matching functions transformed from the 10$^\circ$ cone fundamentals.  Note that $\bar{Y}(\lambda)$ peaks at $1.0$ and the integrated area under all three functions are equal to each other. (generation \href{https://github.com/kyle-c-mcdermott/visualizing-color-space/blob/main/generation/figure_05_color_matching_functions.py}{code} and images \href{https://drive.google.com/drive/folders/1nmZeUppvwRbR2LAT9Mtv4QiCd3mLQVk9?usp=sharing}{folder})}\label{fig:color_matching_functions}
\end{halffigure}
...
\begin{halffigure} % Figure 6
    ...
    \fi
    \captionof{figure}{CIE standard illuminant $D65$ energy spectrum.  (generation \href{https://github.com/kyle-c-mcdermott/visualizing-color-space/blob/main/generation/figure_06_d65_spectrum.py}{code} and images \href{https://drive.google.com/drive/folders/1ImHhNxhM9b3rZ1gdDJlhQYEdZ1GHwRpo?usp=sharing}{folder})}\label{fig:d65_spectrum}
\end{halffigure}
...
\begin{figure*} % Figure 7
    ...
    \fi
    \caption{Illustration of the estimation of chromaticity from a spectrum.  The three left panels show the products of the three color matching functions with the $D65$ spectrum.  The area shaded under each resulting curve is annotated within each panel by applying Equation \ref{eq:tristimulus_from_spectrum}.  The right panel shows the resulting chromaticity coordinate $(x,y)$ for the CIE 170-2 10$^\circ$ space (slightly different from the typically used CIE 1931).  (generation \href{https://github.com/kyle-c-mcdermott/visualizing-color-space/blob/main/generation/figure_07_estimating_d65_chromaticity.py}{code} and images \href{https://drive.google.com/drive/folders/1UFcI16sAtqlmSAhdsDFkcyuREq4bKJcp?usp=sharing}{folder})}\label{fig:estimating_d65_chromaticity}
\end{figure*}

Figure \ref{fig:d65_spectrum} shows the spectrum for the CIE standard illuminant $D65$ for average midday illumination (clear skies in Western / Northern Europe) (\cite{judd1964spectral}) (tabulated \href{http://www.cvrl.org/cie.htm}{here}).  Such spectra will here be represented by $\bar{R}(\lambda)$ generally, and for this specific spectrum $\bar{R}_{D65}(\lambda)$.
...
\begin{halffigure} % Figure 8
    ...
    \fi
    \captionof{figure}{Spectrum loci and $D65$ chromaticities for the CIE 170-2 10$^\circ$ and 2$^\circ$, 1964 10$^\circ$, and 1931 2$^\circ$ standards.  The chromaticity coordinates for $D65$ are given in the legend.  (generation \href{https://github.com/kyle-c-mcdermott/visualizing-color-space/blob/main/generation/figure_08_chromaticity_spaces.py}{code} and images \href{https://drive.google.com/drive/folders/1TnVgKeOvcCuX8cNPWcL-nHwxwh0febu4?usp=sharing}{folder})}\label{fig:chromaticity_spaces}
\end{halffigure}
...
\begin{figure*} % Figure 9
    ...
    \fi
    \caption{CIE 170-2 10$^\circ$ color matching functions with the tristimulus values of the experiment primaries and the $20,250cm^{-1}$ test light annotated along the curves.  (generation \href{https://github.com/kyle-c-mcdermott/visualizing-color-space/blob/main/generation/figure_09_color_matching_experiment_tristimulus.py}{code} and images \href{https://drive.google.com/drive/folders/1bNmSbscUbIGLo2Kqa7EbSv3G7ygZlUgt?usp=sharing}{folder})}\label{fig:color_matching_experiment_tristimulus}
\end{figure*}

\subsection{Color Mixing in the Matching Experiment} % 3.1

The matching of mixed lights in the color matching experiment is easiest to arrange in a chromaticity diagram visually for the range of test colors between the green and blue primaries - where the green and blue settings are well above zero and contributing to the match side, and the red setting is well below zero and contributing to the test side - but the math is the same for any test light used in the experiment.  For this demonstration the test light with a wave-number of $20,250cm^{-1}$ is used, where the observers' averaged red, green, and blue settings are $\bar{R}(20,250cm^{-1})=-0.444$, $\bar{G}(20,250cm^{-1})=0.531$, and $\bar{B}(20,250cm^{-1})=0.144$, respectively.  The green and blue settings are positive and their tristimulus values are added together for the match side; the red setting is negative and its tristimulus values will be added to the test light's tristimulus values on the test side:
...
\begin{figure*} % Figure 11
    ...
    \fi
    \caption{Red, green, and blue phosphor spectra measured from a CRT monitor.  Above each spectral line is also shown the summed spectrum of white (dashed line).  The right panel plots the chromaticities (within the CIE 170-2 10$^\circ$ standard) of the three phosphors and white; the triangle formed by the three primaries is the monitor's color gamut.  All colors that can be shown by combining the three phosphors at various intensities fall within the color gamut triangle.  (generation \href{https://github.com/kyle-c-mcdermott/visualizing-color-space/blob/main/generation/figure_11_crt_monitor.py}{code} and images \href{https://drive.google.com/drive/folders/1ETu-8gwD_q7PnVMK_1vbuLj4S4jq_kwV?usp=sharing}{folder})}\label{fig:crt_monitor}
\end{figure*}
...
\begin{figure*} % Figure 12
    ...
    \fi
    \caption{Saturated surfaces of the sRGB color space cube.  In the left panel, all colors contain at least one 0.0 for red, green, and/or blue.  In the right panel, all colors contain at least one 1.0 for red, green, and/or blue.  (generation \href{https://github.com/kyle-c-mcdermott/visualizing-color-space/blob/main/generation/figure_12_3d_srgb.py}{code} and images \href{https://drive.google.com/drive/folders/1Msyey0Z0dV8eW4WMrhEFrqv5_qltYQbs?usp=sharing}{folder})}\label{fig:3d_srgb}
\end{figure*}
...
\begin{halffigure} % Figure 14
    ...
    \fi
    \captionof{figure}{Top-down view of the sRGB color gamut transformed into chromoluminance space (here two-dimensional chromaticity) which effectively fills the display color gamut with color.  (generation \href{https://github.com/kyle-c-mcdermott/visualizing-color-space/blob/main/generation/figure_14_chromaticity_inside_gamut.py}{code} and images \href{https://drive.google.com/drive/folders/1iSeUYSEwZsFwwevugkakezgTlfnN5oti?usp=sharing}{folder})}\label{fig:chromaticity_inside_gamut}
\end{halffigure}
...
\begin{halffigure} % Figure 16
    ...
    \fi
    \captionof{figure}{Color-filled chromaticity diagram with no gamma correction applied when coloring in the region inside the sRGB color gamut triangle.  Note that the edges of the color gamut triangle are far less apparent.  (generation \href{https://github.com/kyle-c-mcdermott/visualizing-color-space/blob/main/generation/figure_16_chromaticity_without_gamma_correction.py}{code} and images \href{https://drive.google.com/drive/folders/1IkebTXcPb1pfgDt_7uqDshoI3zbplXzk?usp=sharing}{folder})}\label{fig:chromaticity_without_gamma_correction}
\end{halffigure}
...
\begin{figure*} % Figure 17
    ...
    \fi
    \caption{Saturated surfaces transformed into $(x,y,Y)$ chromoluminance space with (left panel) and without (right panel) gamma correction applied.  With gamma correction the apparent brightness of pixels increases with luminance $Y$ uniformly around the color gamut volume.  Without gamma correction the sides generally appear darker, yet the brightness is still high near the primary colors creating variation in apparent brightness around what should be a path of constant luminance.  (generation \href{https://github.com/kyle-c-mcdermott/visualizing-color-space/blob/main/generation/figure_17_gamma_correction_in_chromoluminance.py}{code} and images \href{https://drive.google.com/drive/folders/1fanZNQK85hNcIA-Uc31lcRT7wAxD2Cjt?usp=sharing}{folder})}\label{fig:gamma_correction_in_chromoluminance}
\end{figure*}

\subsection{The Visible Spectrum}\label{sec:visible_spectrum} % 4.4
...
\begin{halffigure} % Figure 18
    ...
    \fi
    \captionof{figure}{Color-filled chromaticity diagram and spectrum band.  (generation \href{https://github.com/kyle-c-mcdermott/visualizing-color-space/blob/main/generation/figure_18_visible_spectrum_locus.py}{code} and images \href{https://drive.google.com/drive/folders/1yegyTBuFW-dKmuYwwyTJAy1dxEdsP2z4?usp=sharing}{folder})}\label{fig:visible_spectrum_locus}
\end{halffigure}
...
\begin{figure*} % Figure 20
    ...
    \fi
    \caption{Blackbody spectra for selected temperatures and their chromaticities plotted along the Planckian locus in CIE 1931 2$^\circ$ $(x,y)$ chromaticity space.  Note that the Planckian locus is not actually drawn out to infinity - it ends at $10^{10}K$ here - but chromaticity changes become infinitesimal as temperatures become infinite.  (generation \href{https://github.com/kyle-c-mcdermott/visualizing-color-space/blob/main/generation/figure_20_blackbody_spectra.py}{code} and images \href{https://drive.google.com/drive/folders/1Ka9sIbll4fDCts8xhtymCu-xvL82BrbM?usp=sharing}{folder})}\label{fig:blackbody_spectra}
\end{figure*}
...
\begin{figure*} % Figure 21
    ...
    \fi
    \caption{CIE 1931 $(x,y)$ (left) and CIE 1960 $(u,v)$ (right) chromaticity diagrams.  The sRGB color gamut triangle is present in both diagrams along with the Planckian locus.  (generation \href{https://github.com/kyle-c-mcdermott/visualizing-color-space/blob/main/generation/figure_21_two_chromaticity_spaces.py}{code} and images \href{https://drive.google.com/drive/folders/1BDUBmb-QsPT8dFZNQNr52lIiOJw501zU?usp=sharing}{folder})}\label{fig:two_chromaticity_spaces}
\end{figure*}
...
\begin{figure*} % Figure 22
    ...
    \fi
    \caption{CIE 1960 $(u,v)$ chromaticity (zoomed in) showing isotherm lines for selected temperatures along the Planckian locus and standard illuminants $A$, $D65$, $E$, and $F3$.  (generation \href{https://github.com/kyle-c-mcdermott/visualizing-color-space/blob/main/generation/figure_22_isotherm_lines_zoom.py}{code} and images \href{https://drive.google.com/drive/folders/1CEhCKWQxPIKy7pBD0pldryHGrzkugU7p?usp=sharing}{folder})}\label{fig:isotherm_lines_zoom}
\end{figure*}

Figure \ref{fig:cie_1931_isotherm} shows the same set of isotherm lines and standard illuminants in CIE 1931 $(x,y)$ chromaticity.  Note that the isotherm lines are not necessarily perpendicular to the Planckian locus and they do not have uniform length; therefore it is always necessary to estimate correlated color temperature within the CIE 1960 $(u,v)$ space in which it is defined even when using a different space for visualization.
...
\begin{halffigure} % Figure 23
    ...
    \fi
    \captionof{figure}{CIE 1931 $(x,y)$ chromaticity showing the same isotherm lines and standard illuminants as in Figure \ref{fig:isotherm_lines_zoom}.  Note that isotherm lines are not perpendicular to the Planckian locus and have varying lengths.  (generation \href{https://github.com/kyle-c-mcdermott/visualizing-color-space/blob/main/generation/figure_23_cie_1931_isotherm.py}{code} and images \href{https://drive.google.com/drive/folders/1MlLWFE8GABbfQZ3KxQGu_AFUaETwRtB8?usp=sharing}{folder})}\label{fig:cie_1931_isotherm}
\end{halffigure}
...
\begin{figure*} % Figure 24
    ...
    \fi
    \caption{The colors from Table \ref{table:varying_l_cone_activation} are plotted in chromaticity space (left) and chromoluminance space (right).  While the changes in long-wavelength-sensitive cone activation result in a straight line path in chromaticity, there are also changes in luminance that follow a slightly curved path.  (generation \href{https://github.com/kyle-c-mcdermott/visualizing-color-space/blob/main/generation/figure_24_single_protan.py}{code} and images \href{https://drive.google.com/drive/folders/1m3kbEADRfl2A1sNfoxsYasWrOZr_7EtK?usp=sharing}{folder})}\label{fig:single_protan}
\end{figure*}
...
\begin{figure*} % Figure 25
    ...
    \fi
    \caption{Multiple protanope (missing long-wavelength-sensitive cone) confusion lines plotted in chromaticity (left) and chromoluminance (right).  The point where the chromaticity lines intersect - the copunctal point - is annotated in the left panel.  Note that in chromoluminance space the confusion line paths approach infinite luminance asymptotically as chromaticity goes toward the copunctal point.  (generation \href{https://github.com/kyle-c-mcdermott/visualizing-color-space/blob/main/generation/figure_25_multiple_protan.py}{code} and images \href{https://drive.google.com/drive/folders/1Zc2HgcApw33guKIdnvmvRdiNLuwgQzR7?usp=sharing}{folder})}\label{fig:multiple_protan}
\end{figure*}
...
\begin{halffigure} % Table 1
    ...
    \end{tabular}
    \captionof{table}{Varying the long-wavelength-sensitive cone activation ($L$ column) while holding the others constant and its effects on display color. (generation \href{https://github.com/kyle-c-mcdermott/visualizing-color-space/blob/main/generation/table_01_varying_l_cone_activation.py}{code})}\label{table:varying_l_cone_activation}
\end{halffigure}
...
\begin{figure*} % Figure 28
    ...
    \fi
    \caption{Color-blind test stimuli with decreasing contrast from left-to-right.  The distributions of chromaticities in the sample stimuli are taken from (from top-to-bottom) long-, medium-, and short-wavelength-sensitive confusion lines.  These stimuli are not calibrated - the reader should not take an ability or inability to perform the associated discrimination task here as an indication of whether or not they may be color-blind.  (generation \href{https://github.com/kyle-c-mcdermott/visualizing-color-space/blob/main/generation/figure_28_color_blind_stimuli_contrast.py}{code} and images \href{https://drive.google.com/drive/folders/1zMvC2Z69KrO1LviBeeyEAbulubTf0qox?usp=sharing}{folder})}\label{fig:color_blind_stimuli_contrast}
\end{figure*}
...
\begin{figure*} % Figure 29
    ...
    \fi
    \caption{A colorful photograph filtered to remove chromatic differences to which an observer with a missing cone type would not be sensitive.  From left-to-right are the original image (\href{https://pixabay.com/photos/flower-field-flowers-field-trees-250016/}{source}), a protanope filtered image, a deuteranope filtered image, and a tritanope filtered image.  Below each image is a chromaticity diagram showing the distribution of chromaticities in the image.  Note that the distributions of filtered images have an arc shape - the center around which the distribution arcs is the corresponding copunctal point for the missing cone type.  (generation \href{https://github.com/kyle-c-mcdermott/visualizing-color-space/blob/main/generation/figure_29_color_blind_filter_photograph.py}{code} and images \href{https://drive.google.com/drive/folders/1PIO1Ng9yXaVw9Li6doC012SHSPCTmDwY?usp=sharing}{folder})}\label{fig:color_blind_filter_photograph}
\end{figure*}
